...
def place_order(request, total=0, quantity=0,):
    ...
    grand_total = 0
    tax = 0
    total, quantity = CartItem.objects.get_total_price_quantity(current_user=current_user)
    tax = (2 * total)/100
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            mt = datetime.today().strftime('%m')
            # d = datetime.date(yr,mt,dt)
            current_date = yr + mt + dt #d.strftime("%Y%m%d") #20210305
